...
export class MetadataAccessorService {
  ...
  ): WorkerEventName | undefined {
    return this.reflector.get(WORKER_ON_EVENT_METADATA, target);
  }

  // TASK DECORATOR

  isTask(target: Type<any> | Function): boolean {
    return this.hasMetadata(TASK_METADATA, target);
  }

  isTaskHandler(target: Type<any> | Function): boolean {
    return this.hasMetadata(TASK_HANDLER_METADATA, target);
  }

  getTaskMetadata(target: Type<any> | Function): string | undefined {
    return this.reflector.get(TASK_METADATA, target);
  }

  private hasMetadata(key: Symbol, target: Type<any> | Function): boolean {
    if (!target) {
      return false;
    }
